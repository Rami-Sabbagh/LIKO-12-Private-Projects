...
local handshake = require'Libraries.Websocket.handshake'
local tools = require'Libraries.Websocket.tools'
local ssl = WEB.luasec("ssl")
...
local receive = function(self)
  ...
  if err and (err == "timeout" or err == "wantread" or err == "wantwrite") then
    return nil,nil,false,0,"Socket Timeout"
  end
